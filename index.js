...
const TOKEN = '8622567320:AAEpOXr5zgSHl9jtdHpOw29zyvBPko1pxyU';
...
function banUser(userId) {
  ...
  const u = users.get(key) || { id: Number(userId), username: '', first_name: '' };
  u.banned = true;
  u.bannedAt = new Date().toISOString();
  users.set(key, u);
  activeReplies.delete(Number(userId));
  activeUserReplies.delete(Number(userId));
  pendingGiveaways.delete(key);
  for (const [chatId, channel] of channels.entries()) {
    if (Number(channel.ownerId) === Number(userId) || Number(channel.addedById) === Number(userId)) {
...
// ====== Command /ban (Khusus Owner) ======
bot.onText(/^\/ban(?:@\w+)?\s+(\d+)$/i, async (msg, match) => {
  if (msg.chat.type !== 'private') return;
  if (!isOwner(msg.from.id)) return;

  const targetId = Number(match[1]);
  if (!targetId) return;
  if (targetId === Number(OWNER_ID)) {
    return bot.sendMessage(msg.chat.id, '⚠️ Owner tidak bisa di-ban.');
  }

  const target = banUser(targetId);
  await bot.sendMessage(
    msg.chat.id,
    `🚫 <b>USER BERHASIL DI-BAN</b>\n\n` +
    `🆔 User ID: <code>${targetId}</code>\n` +
    `👤 User: ${target.username ? '@' + escHtml(target.username) : escHtml(target.first_name || '-')}\n\n` +
    `⛔ Semua fitur bot untuk user ini otomatis ditolak.\n` +
    `📵 Pesan/keluhan tidak akan diteruskan ke owner.\n` +
    `🔕 Channel milik user ini tidak lagi dimonitor.`,
    { parse_mode: 'HTML' }
  );
});
...
bot.on('my_chat_member', async (update) => {
  try {
    if (update.from && isBanned(update.from.id)) return;
