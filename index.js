  ...
  try {
    if (fs.existsSync(CHANNELS_FILE)) {
      ...
      const migrated = data.map((c) => ({
        ...c,
        uniqueId: c.uniqueId || makeChannelUniqueId(),
        ...
        ownerId: c.ownerId || (OWNER_ID ? Number(OWNER_ID) : null),
        addedById: c.addedById || (c.ownerId || (OWNER_ID ? Number(OWNER_ID) : null))
      }));
...
function getUserOwnedChannels(userId) {
  ...
  return Array.from(channels.values()).filter(c =>
    Number(c.ownerId) === id || Number(c.addedById) === id || Number(stats.channels[String(c.id)]?.ownerId) === id
  );
}
