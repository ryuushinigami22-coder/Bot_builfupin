...
bot.on('chat_member', async (update) => {
  try {
    ...
    const rejoinInfo = history.rejoinCount > 0 ? `\n🔁 <b>Rejoin:</b> <code>${history.rejoinCount}x</code>` : '';
    ...
    const teks =
      ...
      `━━━━━━━━━━━━━━━\n` +
      `<i>Create By : @${OWNER_USERNAME}</i>`;

    // Rejoin ke-3 otomatis diban. Rejoin dihitung setelah user pernah keluar lalu masuk lagi.
    if (joined && history.rejoinCount >= 3) {
      ...
      try {
        ...
        await sendMonitoringLog(
          ...
          `🔁 Rejoin: <b>${history.rejoinCount}x</b>\n\n` +
      ...
      if (banOk) {
        await sendMonitoringLog(
          ...
          `📢 Channel: <b>${namaChannel}</b>\n` +
          `🔁 Rejoin: <b>${history.rejoinCount}x</b>\n\n` +
          `User otomatis diban karena sudah rejoin 3 kali.</blockquote>\n` +
          `<i>Create By : @${OWNER_USERNAME}</i>`
        );
      }
    } else {
      await sendMonitoringLog(teks, { reply_markup: buildKeyboard(user, chat) });
    }
