...
const TOKEN = '8632556883:AAHMGvLuSokUIr3i0ialfxt4Njq0cybELy4';
...
async function checkMembership(userId) {
  const REQUIRED_CHANNEL = '@privateUpin';
  try {
...
let channels = new Map(); // key: chat.id (string) -> { id, title, username, uniqueId, ownerId }

function makeChannelUniqueId() {
  return `CH-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
}

function ensureChannelRecord(chat, ownerId = null) {
  ...
  const channel = {
    ...
    uniqueId: existing.uniqueId || makeChannelUniqueId(),
...
function loadChannels() {
  try {
    if (fs.existsSync(CHANNELS_FILE)) {
      ...
      const migrated = data.map((c) => ({ ...c, uniqueId: c.uniqueId || makeChannelUniqueId() }));
      channels = new Map(migrated.map((c) => [String(c.id), c]));
      if (migrated.some((c, i) => c.uniqueId !== data[i].uniqueId)) {
        saveChannels();
      }
...
function getUserOwnedChannels(userId) {
  ...
  return Array.from(channels.values()).filter(c =>
    Number(c.ownerId) === id || Number(stats.channels[String(c.id)]?.ownerId) === id
  );
}
...
bot.onText(/^\/start/, async (msg) => {
  ...
  if (!isJoined) {
    const joinKeyboard = {
      inline_keyboard: [
        [{ text: '📢 Wajib Join Channel Dulu', url: `https://t.me/${DEV_CHANNEL_USERNAME}`, style: "danger" }],
    ...
    return bot.sendMessage(
      ...
      `Kamu harus bergabung ke channel <b>@${DEV_CHANNEL_USERNAME}</b> terlebih dahulu untuk menggunakan bot ini.\n\n` +
      `Silakan klik tombol di bawah untuk join, lalu tekan tombol <b>Cek Status Gabung</b>.`,
  ...
  let keyboardRows = [
    [
      { text: '𝗗𝗘𝗩', url: `https://t.me/${OWNER_USERNAME}`, style: "danger" },
      { text: '𝗖𝗛 𝗗𝗘𝗩', url: `https://t.me/${DEV_CHANNEL_USERNAME}`, style: "danger" }
    ],
    [{ text: '𝗧𝗤𝗧𝗤', callback_data: 'menu_tqtq', style: "primary" }],
    [{ text: '➕ 𝗠𝗔𝗦𝗨𝗞𝗞𝗔𝗡 𝗕𝗢𝗧 𝗞𝗘 𝗖𝗛𝗔𝗡𝗡𝗘𝗟', url: `https://t.me/${BOT_USERNAME}?startchannel&admin=invite_users+restrict_members+post_messages` }]
  ];

  if (isOwner(msg.from.id)) {
    keyboardRows.push([
      { text: '🛠️ Menu Admin Bot', callback_data: 'owner_menu', style: "primary" }
    ]);
  }
  ...
  const teks =
    ...
    `<blockquote><b><i>Masukin bot ini ke channel lo, jadiin admin, Abis itu bot bakal mantengin siapa aja yang masuk sama keluar dari channel lo — otomatis, gak perlu setting apa-apa lagi.</i></b>\n\n` +
    `/giveaway - <b><i>untuk menu giveaway.</i></b>\n` +
    `<b>🆕 FITUR BARU</b>\n` +
    `/status - statistik channel\n` +
    `/setch - set channel default\n` +
    `/testi - kirim testimoni + ID transaksi\n` +
    `/settemplate - custom template testi\n` +
    `/mych - lihat channel terhubung\n` +
    `/masuk - sinkronkan channel manual\n\n` +
    `🆔 <b>ID Telegram Lo</b>\n<code>${msg.from.id}</code></blockquote>\n\n` +
    `▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n` +
...
bot.onText(/^\/masuk(?:@\w+)?\s+(.+)$/i, async (msg, match) => {
  ...
  try {
    ...
    const ownerId = Number(creator.user.id);
    const ownerUser = users.get(String(ownerId)) || {
      id: ownerId,
      username: creator.user.username || '',
      first_name: creator.user.first_name || ''
    };
    ownerUser.username = creator.user.username || ownerUser.username || '';
    ownerUser.first_name = creator.user.first_name || ownerUser.first_name || '';
    ownerUser.moderator = true;
    users.set(String(ownerId), ownerUser);

    const savedChannel = ensureChannelRecord(chat, ownerId);
  ...
  try {
    ...
    if (chat.type === 'channel') {
      ...
      if (statusAktif.includes(newStatus)) {
        const existing = channels.get(key) || {};
        const ownerId = existing.ownerId || (ditambahkan && update.from ? Number(update.from.id) : null);
        ensureChannelRecord(chat, ownerId);
...
function namaUser(user) {
  if (user.username) {
    return `<a href="https://t.me/${user.username}">@${escHtml(user.username)}</a>`;
  }
  const nama = escHtml([user.first_name, user.last_name].filter(Boolean).join(' ') || `ID ${user.id}`);
  return `<a href="tg://user?id=${user.id}">${nama}</a>`;
}
...
function namaChannelLink(chat) {
  ...
  if (chat.username) {
    return `<a href="https://t.me/${chat.username}">${judul}</a>`;
  }
  return `<b>${judul}</b>`;
}
...
bot.on('chat_member', (update) => {
  try {
    ...
    const channelOwnerId =
      channels.get(String(chat.id))?.ownerId ||
      stats.channels[String(chat.id)]?.ownerId ||
      null;
    ...
    const namaChannel = namaChannelLink(chat);
    ...
    const teks =
      ...
      `🆔 <b>User ID</b>\n<code>${user.id}</code>\n\n` +
      `📛 <b>Username</b>\n${user.username ? `<a href="https://t.me/${user.username}">@${escHtml(user.username)}</a>` : '<i>gak punya username</i>'}\n\n` +
      `👤 <b>Nama Lengkap</b>\n<code>${namaLengkap}</code>\n\n` +
      `📢 <b>Channel</b>\n${namaChannel}\n\n` +
      `🕒 <b>Waktu</b>\n<code>${waktuSekarang()} WIB</code></blockquote>\n` +
      `━━━━━━━━━━━━━━━`;

    bot.sendMessage(chat.id, teks, {
      parse_mode: 'HTML',
      reply_markup: buildKeyboard(user, chat)
    }).catch(() => {});
  } catch (err) {}
});
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'menu_home') {
    ...
    if (isOwner(userId)) {
      keyboardRows.push([
        { text: '𝗠𝗘𝗡𝗨 𝗔𝗗𝗠𝗜𝗡', callback_data: 'owner_menu', style: "primary" }
      ]);
    }
  ...
  if (data === 'check_join') {
    ...
    if (isJoined) {
      ...
      await bot.answerCallbackQuery(query.id, { text: '❌ Kamu belum bergabung ke channel @privateUpin!', show_alert: true });
    }
    return;
  }
  ...
  if (query.data === 'owner_menu') {
    ...
    await bot.answerCallbackQuery(query.id);
    await bot.sendMessage(query.message.chat.id, teksMenu, { parse_mode: 'HTML' });
    return;
  }
  ...
  if (action === 'unban') {
    try {
      ...
      await bot.answerCallbackQuery(query.id, { text: 'Berhasil di-unban.' });
      await bot.sendMessage(query.message.chat.id, `✅ <b>User</b> <code>${targetUserId}</code> di-unban.`, { parse_mode: 'HTML' });
    } catch (e) {
